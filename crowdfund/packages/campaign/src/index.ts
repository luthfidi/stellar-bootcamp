...
export interface Client {
   ...
   */
  initialize: ({owner, title, description, goal, deadline, category, xlm_token}: {owner: string, title: string, description: string, goal: i128, deadline: u64, category: string, xlm_token: string}, options?: {
    /**
  ...
  get_metadata: (options?: {
     ...
     */
    simulate?: boolean;
  }) => Promise<AssembledTransaction<CampaignMetadata>>

  /**
   * Construct and simulate a get_donation_history transaction. Returns an `AssembledTransaction` object which will have a `result` field containing the result of the simulation. If this transaction changes contract state, you will need to call `signAndSend()` on the returned object.
   * Get donation history (paginated)
   * Returns last N donations based on limit and offset
   */
  get_donation_history: ({limit, offset}: {limit: u32, offset: u32}, options?: {
    /**
     * The fee to pay for the transaction. Default: BASE_FEE
     */
    fee?: number;
   ...
   */
  withdraw: ({owner}: {owner: string}, options?: {
    /**
     * The fee to pay for the transaction. Default: BASE_FEE
     */
    fee?: number;
...
export class Client extends ContractClient {
  ...
  constructor(public readonly options: ContractClientOptions) {
    super(
      new ContractSpec([ "AAAAAQAAAAAAAAAAAAAADkRvbmF0aW9uUmVjb3JkAAAAAAADAAAAAAAAAAZhbW91bnQAAAAAAAsAAAAAAAAABWRvbm9yAAAAAAAAEwAAAAAAAAAJdGltZXN0YW1wAAAAAAAABg==",
  ...
  public readonly fromJSON = {
    initialize: this.txFromJSON<null>,
        ...
        get_metadata: this.txFromJSON<CampaignMetadata>,
        get_donation_history: this.txFromJSON<Array<DonationRecord>>,
